...
%% ******************************** 局部最优1 *********************************
% y = y0(:,2);         % y 用于表示当前各站点的发案数
% for i = 1:length(left_case_Big)
%     a = left_case_Big(i,1);     % 路口序号
%     b = left_case_Big(i,2);     % 路口发案数
%     left_row_index = find(left_from_index == a);  % 路口所在 left_from 的第几行
%     left_row = left_from{left_row_index,2};     % 路口所属于的站点

%     sele = left_row(find(y(left_row) == min(y(left_row))));     % 选择最小的发件序号
%     if length(sele) ~= 1
% %         sele
%         sele = sele(1);
%     end
%     y(sele) = y(sele) + b;
%     belong{sele,2} = [belong{sele,2},a];
%     belong{sele,2} = sort(belong{sele,2});
% end
% std(y,1)
...
%     temp_ys = [];
%     for j = 1:length(left_row)
%         temp_y = y;
%         temp_y(left_row(j)) = temp_y(left_row(j)) + b;
%         temp_ys(j) = std(temp_y,1);
%     end
%     sele = left_row(find(temp_ys == min(temp_ys)));

%     % temp_y = repmat(y,1,length(left_row));
%     % sele = left_row(find(y(left_row) == min(y(left_row))));     % 选择最小的发件序号
%     if length(sele) ~= 1
% %         sele
%         sele = sele(1);
...
%     y = y0(:,2);
%     temp_belong = belong;
%     for i = 1:length(left_from)
%         a = left_from{i,2};     % 当前路口对应站口
%         index_r = ceil(length(a) * rand());
%         b = a(index_r);     % 添加站口序号
%         c = lcase(left_from{i,1});       % 事件数
%         y(b) = y(b) + c;
%         temp_belong{b,2} = [temp_belong{b,2},left_from{i,1}];
%         temp_belong{b,2} = sort(temp_belong{b,2});
%     end
